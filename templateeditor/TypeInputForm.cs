...
namespace ProjectTemplateEditor
{
    ...
    public class TypeInputForm : System.Windows.Forms.Form
    {
        ...
        public ValueType Result
        {
            get
            {
                if (this.listBox.Text == "Text")
                {
                    return ValueType.Text;
                }
                else if (this.listBox.Text == "Boolean (True/False)")
                {
                    return ValueType.Boolean;
                }
                else if (this.listBox.Text == "Option List")
                {
                    return ValueType.OptionList;
                }
                else if (this.listBox.Text == "File Browser")
                {
                    return ValueType.FilePath;
                }
                else if (this.listBox.Text == "Folder Browser")
                {
                    return ValueType.FolderPath;
                }
                else if (this.listBox.Text == "Integer")
                {
                    return ValueType.Integer;
                }
                else
                {
                    throw new ApplicationException("Unexpected listbox value");
                }
            }
        }
        ...
        {
            this.btnOK.Enabled = this.listBox.SelectedIndex != -1;
        }
    }
}
